...
# Load Annual data
Annual_Data <- read_csv("C:/Users/FrixosLarkos/OneDrive - FE International/Desktop/Educreations Metrics/Annual_converted.csv",
                        col_types = cols(Date = col_date(format = "%d/%m/%Y"),
                                         Date_Accrual = col_date(format = "%d/%m/%Y"))) %>% 
  select(customer_email = "Customer.Email", Period = "Annual.or.Monthly", Date_Accrual, Accrual_Amount)

# Load Monthly data
Monthly_Data <- read_csv("C:/Users/FrixosLarkos/OneDrive - FE International/Desktop/Educreations Metrics/Monthly Transactions.csv",
                         col_types = cols(Date = col_date(format = "%d/%m/%Y"))) %>%
  mutate(Date_Accrual = ymd(paste0(year(Date),"-",month(Date),"-",1))) %>% 
  select(customer_email = "Customer Email", Period = "Annual or Monthly", Date_Accrual, Accrual_Amount= Net_Amount)
...
write.csv(Joint_Data, "Metrics_v5.csv", row.names = FALSE)
